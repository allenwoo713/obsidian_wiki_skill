...
用法：python query.py <project_root> "<query>" [--k 5] [--max-tokens 4096] [--json]
"""
...
def hybrid_search(wi, query: str, k: int = 5, max_tokens: int = 4096,
                  wiki_dir: Path = None) -> List[RetrievedPage]:
    ...
    return budget_control(fused[:k * 3], max_tokens=max_tokens)[:k]


def format_for_agent(results: List[RetrievedPage]) -> str:
    if not results:
        return "[无检索结果]"
    lines = ["## 检索结果（hybrid FTS+RAG）\n"]
...
def main():
    if len(sys.argv) < 3:
        print("用法: python query.py <project_root> <query> [--k 5] [--max-tokens 4096] [--json]")
        sys.exit(1)
    ...
    wi.load()
    results = hybrid_search(wi, query, k=k, max_tokens=max_tokens, wiki_dir=proj / "Wiki")
    if as_json:
        print(json.dumps([{
