...
**固定向量索引契约（Phase 7 / issue #49）：** 生产 dense 检索固定为源码控制 `eval/ann-policy.json` 的唯一批准策略——`IVF_HNSW_SQ`（cosine、`num_partitions=1`、`m=20`、`ef_construction=300`）+ 查询 `ef=300`，held-out floors Recall@10 ≥ 0.19 / Recall@20 ≥ 0.17。**没有运行时类型/ef/exact 选择，也没有 exact 回退**：exact 仅为诊断 API。每次构建在发布前用确定性 held-out 验证 query（数量 = `min(BENCHMARK_MAX_PROBES, dense_rows)`）对照 batch-exact 真值校验 recall，不达标即拒绝发布并保留旧 `ACTIVE_INDEX`。旧 format-5 mode-ambiguous 索引加载即报 `RebuildRequiredError`，需全量重建（`--vector-index` 参数已废弃，传入即 exit 2）。Phase 7 由用户选择 `m=20` / `ef_construction=300` / 查询 `ef=300`；最终 exact-head 远程门禁验证该选择，既有决策证据 digest 与 URL 保持不变。策略校验：`python eval/run_eval.py --validate-ann-policy`。
...
#### 预算口径（token 预算契约，agent 必读）

`query.py` 的 token 预算是**分层策略**而非固定上限。`--max-tokens` 只是**基础预算**；最终 ContextBundle 上限由意图倍率决定：

| 意图 | context_mode | 倍率 | 说明 |
|---|---|---|---|
| comparison（对比分析） | multiple_sections | **1.4×** | 需并列多页多 section，扩展预算 |
| lookup / procedure / relation / 其他 | section / parent_section / evidence / … | **1.0×** | 保持基础预算 |

即：`effective = min( 基础预算 × 意图倍率 , 硬上限（若提供） )`。

- `comparison` 查询（"A vs B 区别/差异/对比"）→ `--max-tokens 4096` 实际得到 `effective_budget_tokens = 5734`。
- 其它意图 → `effective_budget_tokens == 基础预算`。
- 显式 `--mode full/snippet/summary` 覆盖时，倍率强制为 `1.0`（按基础预算封顶）。

**agent 调用 LLM 前，必须按输出里的 `effective_budget_tokens` 预留上下文，而不是想当然按 `--max-tokens` 预留**——否则 comparison 场景会低估占用，导致上下文被宿主截断。JSON 顶层已暴露 6 个预算字段：`requested_base_budget_tokens` / `budget_multiplier` / `effective_budget_tokens` / `hard_max_tokens` / `budget_policy` / `max_context_tokens`（其中 `max_context_tokens` 恒等于 `effective_budget_tokens`，仅为向后兼容保留）。

当宿主模型窗口或成本有硬约束时，传 `--hard-max-tokens N`：仍以 `multiple_sections` 取材，但 `effective_budget_tokens` 与 `token_count` 都不会超过 N，超出部分会显式标注 `truncated` / `truncation_reason` / omitted range，绝不静默丢弃。

> 预算由 `query.py` 的 `resolve_budget()` 唯一决定并经 `ContextBundle` 暴露；下游（eval / 宿主）只可读 `effective_budget_tokens`，**不得复制倍率表**重新推算。
